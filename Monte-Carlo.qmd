...
where $N(s,a)$ is the number of episodes that visit state-action pair $(s,a)$ at least once , and $G_k(s,a)$ is the return following the first visit to state-action pair $(s,a)$ in the $k$-th episode.
Let $t_k(s,a)$ be the first time in episode $k$ that state-action pair $(s,a)$ is visited, and $T_k$ be the length of the $k$-th episode. $G_k(s,a)$ is the sum of the discounted rewards of this episode,  starting from time step $t_k(s,a)+1$ to $T_k$. 

$$
G_k(s,a) = R_{t_k(s,a)+1} + \gamma R_{t_k(s,a)+2} + \dots + \gamma^{T_k-t_k(s,a)-1} R_{T_k}
$$

The average over the episodes is computed incrementally. To implify notation, we concentrate on a single state-action pair $(s,a)$. Let $Q_n$ be the estimate of the state-action value after $n-1$ first visits.
...
$$
\begin{aligned}
Q_{n+1} &= \frac{1}{n} \sum_{k=1}^{n} G_k \\
&= \frac{1}{n} \left( \sum_{k=1}^{n-1} G_k + G_n \right) \\
&= \frac{1}{n} \left( (n-1)Q_n + G_n \right) \\
...
The figure below shows the optimal policy found, which is essentially the same as the one in Figure \ref{fig:mc-exploring-starts}. The crucial observation is that the agent corectly learned the strategy to avoid the pit. 

```{python}
#| label: fig-mc-optimal-policy
#| fig-cap: "Optimal policy found by Monte Carlo with Exploring Starts"
#| echo: false
